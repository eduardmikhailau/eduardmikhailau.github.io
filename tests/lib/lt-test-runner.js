const config = window.ltTestConfig;
if (!config || !config.wordSet || !config.gapFillVariants) {
  throw new Error("Missing ltTestConfig with wordSet and gapFillVariants.");
}

const testTitle = config.title || document.title || "Practice Tests";
...
document.body.innerHTML = `
  <div class="mx-auto flex min-h-screen max-w-3xl flex-col px-4 py-6">
    <header class="mb-6 text-center">
      <h1 id="page-title" class="text-2xl font-semibold sm:text-3xl"></h1>
    </header>
    ...
    <main class="flex-1">
      <section id="start-screen" class="space-y-4 rounded-2xl bg-white p-6 shadow-sm">
        ...
        <p id="version-note" class="hidden text-xs text-amber-600">
          Saved progress was reset because this test was updated.
        </p>
        <button
          id="start-button"
...
document.getElementById("page-title").textContent = testTitle;
...
const TEST_VERSION = config.testVersion || "1.0.0";
...
const getRandomWords = (excludeId, count, field = "lt") => {
  return shuffle(wordSetData.words.filter((item) => item.id !== excludeId))
    .slice(0, count)
    .map((item) => item[field]);
};
...
const buildGapFill = () => {
  const questions = [];
  wordSetData.words.forEach((word) => {
    const variants = gapFillVariantsData[word.id] || [];
...
const loadSession = () => {
  ...
  try {
    ...
    if (isOutdated) {
      clearSavedSession();
      return { session: null, isOutdated: true };
    }
...
const updateStartScreen = () => {
  ...
  const showOutdated = saved.isOutdated;
